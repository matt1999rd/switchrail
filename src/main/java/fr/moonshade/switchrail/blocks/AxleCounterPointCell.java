...
public class AxleCounterPointCell implements IPanelCell,ICounterHandler,ICounterPoint, ISRCell, IPanelCellInfoProvider {
    ...
    private void renderRedstonePoint(VertexConsumer builder, PoseStack stack, int combinedLight, float alpha) {
        ...
        stack.translate(0.4375F,0.4375F,0.125F); // 7 7 2
        SRRenderHelper.drawRectangle(builder,spriteRedstone,stack,0,0.125F,0,0.125F,0,2,0,2,combinedLight,0xFFFFFFFF,alpha); // x1 = y1 = 0 x2 = y2 = 2/16
        stack.popPose();
        // back / north
        stack.pushPose();
        SRRenderHelper.rotate(stack,Direction.Axis.X,-90);
        stack.translate(0,-0.125F,1); // 0 -2 16
    ...
    private void renderBase(VertexConsumer builder, PoseStack stack, int combinedLight, float alpha) {
        ...
        stack.pushPose();
        stack.translate(0,0,0.125F); // 0 0 2
        SRRenderHelper.drawRectangle(builder,SRRenderHelper.SPRITE_AXLE_COUNTER,stack,0,1,0,1,combinedLight,0xFFFFFFFF,alpha); // x1 = y1 = 0, x2 = y2 = 16
        stack.popPose();
        // bottom / down
        stack.pushPose();
