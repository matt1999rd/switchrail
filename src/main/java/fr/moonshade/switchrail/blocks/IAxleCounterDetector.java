...
public interface IAxleCounterDetector {
    ...
    default void onMinecartLimitPass(Level world, WorldCounterPoints worldCP, BlockPos pos,Direction side,boolean isLeaving){
        ...
        List<CounterPoint> counterPointsOfNonExistingAC = new ArrayList<>();
        ...
        if (!sideCPoints.isEmpty()){
            sideCPoints.forEach(cp->{
                ...
                if (handler == null){
                    counterPointsOfNonExistingAC.add(cp);
                    Logger.getGlobal().warning("Skip call to add/remove axle because counter point is non existant !");
                } else if (addAxle){
                    handler.addAxle();
        ...
        if (!counterPointsOfNonExistingAC.isEmpty()){
            counterPointsOfNonExistingAC.forEach(cp->{
                worldCP.onACRemove(cp.getACPos(),cp.getIndex());
            });
        }
    }
